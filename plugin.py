...
# ==========================================================
# دالة الهاش الاحترافية المتوافقة تماماً مع Oscam/Ncam
# ==========================================================
crc_table = array("L")
...
class BISSPro(Screen):
    ...
    def manual_done(self, key=None):
        ...
        if self.save_biss_key(full_id, key, info.getName()): self.res = (True, f"Saved: {info.getName()} | Hash: {full_id}")
...
class HexInputScreen(Screen):
    ...
    def get_active_channel_data(self):
        ...
        if service:
            ...
            # عرض الهاش الاحترافي في شاشة الإدخال
            display_hash = get_oscam_hash(t_data.get("namespace", 0), t_data.get("transport_stream_id", 0), t_data.get("original_network_id", 0), sid)
            self["channel_data"].setText(f"{int(freq)} {pol} {sr} | Hash: {display_hash}")
