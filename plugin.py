...
# ==========================================================
# دالة الهاش الاحترافية - مطابقة 100% لصورتك (تنتج 17E679FE)
# ==========================================================
def get_oscam_hash(namespace, tsid, onid, sid):
    try:
        ...
        # الترتيب Big-endian (>) وتصفير أول 16 بت من الـ Namespace
...
# ==========================================================
# الإعدادات والمسارات - نسخة v1.2 الاحترافية
# ==========================================================
...
class BISSPro(Screen):
    ...
    def manual_done(self, key=None):
        ...
        if self.save_biss_key(h, key, info.getName()): self.res = (True, f"Saved Hash: {h}")
    ...
    def save_biss_key(self, h, key, name):
        ...
        try:
            ...
            lines.append(f"F {h.upper()} 00000000 {key.upper()} ;{name} | {current_date}\n")
    ...
    def do_update(self):
        try:
            req = urllib.request.Request("https://raw.githubusercontent.com/anow2008/softcam.key/main/softcam.key")
    ...
    def do_auto(self, service):
        try:
            ...
            found = False
            try:
                resp = urllib.request.urlopen(urllib.request.Request(GOOGLE_SHEET_URL, headers=headers), timeout=8).read().decode("utf-8").splitlines()
            ...
            if not found: self.res = (False, "Key not found on server")
            ...
            for row in csv.reader(resp):
                if len(row) >= 2:
                    ...
                    if len(nums) >= 2 and abs(curr_freq - int(nums[0])) <= 3 and curr_pol in row[0].upper():
                        ...
                        if len(clean) == 16:
                            self.save_biss_key_background(h, clean, ch_name)
                            break
        except: pass
        self.is_scanning = False
        ...
        try:
            ...
            if os.path.exists(target):
                with open(target, "r") as f:
                    for line in f:
                        if f"F {h.upper()}" not in line.upper(): lines.append(line)
            lines.append(f"F {h.upper()} 00000000 {key.upper()} ;{name} (Auto) | {current_date}\n")
            with open(target, "w") as f: f.writelines(lines)
            restart_softcam_global()
...
class BissManagerList(Screen):
    def __init__(self, session):
        ...
        <screen position="center,center" size="{self.ui.px(1000)},{self.ui.px(700)}" title="BissPro - Key Editor">
            ...
            <eLabel position="{self.ui.px(30)},{self.ui.px(590)}" size="{self.ui.px(30)},{self.ui.px(30)}" backgroundColor="#ff0000" />
            <eLabel text="RED: Delete" position="{self.ui.px(75)},{self.ui.px(585)}" size="{self.ui.px(300)},{self.ui.px(40)}" font="Regular;26" transparent="1" />
        </screen>"""
...
class HexInputScreen(Screen):
    def __init__(self, session, channel_name="", existing_key=""):
        ...
        <screen position="center,center" size="{self.ui.px(1150)},{self.ui.px(650)}" title="BissPro - Key Input" backgroundColor="#1a1a1a">
            ...
            <widget name="char_list" position="{self.ui.px(1020)},{self.ui.px(120)}" size="{self.ui.px(100)},{self.ui.px(300)}" font="Regular;{self.ui.font(45)}" halign="center" transparent="1" />
            ...
            <widget name="l_red" position="{self.ui.px(115)},{self.ui.px(495)}" size="{self.ui.px(150)},{self.ui.px(40)}" font="Regular;26" />
            <eLabel position="{self.ui.px(330)},{self.ui.px(500)}" size="{self.ui.px(25)},{self.ui.px(25)}" backgroundColor="#00ff00" />
            <widget name="l_green" position="{self.ui.px(365)},{self.ui.px(495)}" size="{self.ui.px(150)},{self.ui.px(40)}" font="Regular;26" />
        </screen>"""
...
def sessionstart(reason, session=None, **kwargs):
    ...
    if reason == 0 and session: watcher_instance = BissProServiceWatcher(session)
