...
# ==========================================================
# دالة الهاش الاحترافية - بتطلع الهاش اللي بيفتح القنوات فعلاً
# ==========================================================
def get_biss_crc32(sid, freq):
    # تحويل التردد لـ MHz والـ SID لـ Hex ودمجهم بطريقة الـ MPEG
    # دي الطريقة اللي بتطلع هاشات زي 9DE5E789
...
# ==========================================================
# الإعدادات والمسارات - النسخة الأصلية 500 سطر
# ==========================================================
...
class BISSPro(Screen):
    ...
    def update_dynamic_logo(self):
        if not self.instance or "main_logo" not in self or not self["main_logo"].instance: return
        ...
        try:
            ...
            if remote_search:
                ...
                if remote_v > 1.0:
                    ...
                    except: notes = "New update available."
                    msg = "Update Found: v%s\n\n%s\n\nInstall?" % (str(remote_v), notes)
    ...
    def do_plugin_download(self):
        try:
            import ssl
            ctx = ssl._create_unverified_context()
            req = urllib.request.Request(URL_PLUGIN + "?nocache=" + str(random.randint(1000, 9999)), headers={'User-Agent': 'Mozilla/5.0'})
            ...
            if len(new_code) > 2000:
                ...
                with open(target_file, "wb") as f: f.write(new_code)
                self.res = (True, "Plugin Updated! Restart Enigma2?", "plugin_upd")
            else: self.res = (False, "Download failed.")
    ...
    def show_result(self): 
        ...
        if self.res[0]:
            if len(self.res) > 2 and self.res[2] == "plugin_upd": self.session.openWithCallback(self.answer_restart, MessageBox, self.res[1], MessageBox.TYPE_YESNO)
    ...
    def build_menu(self):
        ...
        menu_items = [("Add Key", "Manual BISS Entry", "add", icon_dir + "add.png"), ("Key Editor", "Manage stored keys", "editor", icon_dir + "editor.png"), ("Download Softcam", "Full update from server", "upd", icon_dir + "Download Softcam.png"), ("Autoroll", "Smart search for current channel", "auto", icon_dir + "auto.png")]
        lst = []
    ...
    def do_update(self):
        try:
            ...
            os.chmod(get_softcam_path(), 0o644); restart_softcam_global(); self.res = (True, "Updated")
        ...
        try:
            ...
            for row in csv.reader(resp):
                if len(row) >= 2 and str(freq) in row[0]:
                    ...
                    if len(key) == 16: self.save_biss_key(crc_id, key, info.getName()); found = True; break
            if found: self.res = (True, f"Found: {info.getName()}"); 
        ...
        try:
            ...
            if os.path.exists(target):
                with open(target, "r") as f:
                    for line in f:
                        if f"F {full_id.upper()}" not in line.upper(): lines.append(line)
            lines.append(f"F {full_id.upper()} 00000000 {key.upper()} ;{name} (Auto)\n")
            with open(target, "w") as f: f.writelines(lines)
...
class BissManagerList(Screen):
    def __init__(self, session):
        ...
        self.skin = f"""<screen position="center,center" size="{self.ui.px(1000)},{self.ui.px(700)}" title="BissPro - Key Editor">
            <widget name="keylist" position="20,20" size="960,520" itemHeight="50" scrollbarMode="showOnDemand" />
        </screen>"""
        self["keylist"] = MenuList([]); self["actions"] = ActionMap(["OkCancelActions", "ColorActions"], {"cancel": self.close}, -1)
        self.onLayoutFinish.append(self.load_keys)
    def load_keys(self):
        p = get_softcam_path(); keys = []
        if os.path.exists(p): keys = [l.strip() for l in open(p).readlines() if l.startswith("F ")]
...
class HexInputScreen(Screen):
    def __init__(self, session, channel_name="", existing_key=""):
        self.ui = AutoScale()
        Screen.__init__(self, session)
        self.skin = f"""<screen position="center,center" size="{self.ui.px(1150)},{self.ui.px(650)}" title="BissPro - Key Input" backgroundColor="#1a1a1a">
            <widget name="channel" position="0,20" size="1150,60" font="Regular;45" halign="center" foregroundColor="#00ff00" transparent="1" />
            <widget name="progress" position="175,90" size="800,10" foregroundColor="#00ff00" />
            <widget name="keylabel" position="0,120" size="1150,110" font="Regular;80" halign="center" foregroundColor="#f0a30a" transparent="1" />
            <widget name="channel_data" position="0,280" size="1130,50" font="Regular;32" halign="center" foregroundColor="#ffffff" transparent="1" />
            <widget name="char_list" position="1020,120" size="100,300" font="Regular;45" halign="center" transparent="1" />
        </screen>"""
        self["channel"] = Label(channel_name); self["channel_data"] = Label(""); self["keylabel"] = Label(""); self["char_list"] = Label(""); self["progress"] = ProgressBar()
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "NumberActions", "DirectionActions"], {
            "cancel": self.close, "green": self.save, "ok": self.confirm_char, "left": self.move_left, "right": self.move_right, "up": self.move_char_up, "down": self.move_char_down,
        ...
        self.key_list = list(existing_key.upper()) if len(existing_key) == 16 else ["0"] * 16
    ...
    def get_active_channel_data(self):
        ...
        if s:
            ...
            self["channel_data"].setText(f"{int(f)} | SID: %04X" % (info.getInfo(iServiceInformation.sSID)&0xFFFF))
    def update_display(self):
        res = ""; 
        for i in range(16): res += ("[%s]" if i==self.index else "%s") % self.key_list[i]
        self["keylabel"].setText(res); self["progress"].setValue(int(((self.index+1)/16.0)*100))
        c_res = ""; 
        for i, c in enumerate(self.chars): c_res += ("\c00f0a30a[%s]\n" if i==self.char_index else " %s \n") % c
...
def sessionstart(reason, session=None, **kwargs):
    ...
    if reason == 0 and session and watcher_instance is None: watcher_instance = BissProServiceWatcher(session)
