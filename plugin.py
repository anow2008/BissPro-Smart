...
# ==========================================================
# دالة CRC32 - مدمجة بناءً على طلبك
# ==========================================================
...
def restart_softcam_global():
    ...
    for s in scripts:
        if os.path.exists(s): os.system(f"{s} restart >/dev/null 2>&1")
...
class BISSPro(Screen):
    ...
    def thread_check_version(self):
        try:
            ...
            remote_data = urllib.request.urlopen(urllib.request.Request(v_url, headers={'User-Agent': 'Mozilla/5.0'}), timeout=10, context=ctx).read().decode("utf-8")
    ...
    def do_plugin_download(self):
        try:
            new_code = urllib.request.urlopen(URL_PLUGIN, timeout=30).read()
    ...
    def save_biss_key(self, full_id, key, name):
        ...
        try:
            current_date = time.strftime("%d-%m-%Y"); lines = []
    ...
    def do_update(self):
        try:
            ...
            restart_softcam_global(); self.res = (True, "Updated Softcam")
        ...
        try:
            ...
            for row in csv.reader(resp):
                if len(row) >= 2 and str(freq) in row[0]:
                    ...
                    if len(key) == 16:
                        self.save_biss_key(crc_id, key, info.getName()); found = True; break
...
class BissProServiceWatcher:
    ...
    def save_biss_key_background(self, full_id, key, name):
        ...
        try:
            ...
            with open(target, "w") as f: f.writelines(lines)
            restart_softcam_global(); addNotification(MessageBox, f"Found: {name}", type=MessageBox.TYPE_INFO, timeout=3)
        except: pass
...
class BissManagerList(Screen):
    def __init__(self, session):
        ...
        self["keylist"] = MenuList([]); self["actions"] = ActionMap(["ColorActions", "OkCancelActions"], {"green": self.edit_key, "red": self.delete_confirm, "cancel": self.close}, -1)
...
class HexInputScreen(Screen):
    def __init__(self, session, channel_name="", existing_key=""):
        ...
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "NumberActions", "DirectionActions"], {
            "cancel": self.close, "red": self.close, "green": self.save, "yellow": self.clear_current, "blue": self.reset_all,
    ...
    def get_active_channel_data(self):
        ...
        if service:
            info = service.info(); t_data = info.getInfoObject(iServiceInformation.sTransponderData)
            freq = t_data.get("frequency", 0); pol = "V" if t_data.get("polarization", 0) else "H"
            self["channel_data"].setText(f"{int(freq/1000)} {pol} | SID: %04X" % (info.getInfo(iServiceInformation.sSID)&0xFFFF))
    def update_display(self):
        res = ""; [res := res + (f"[{c}]" if i == self.index else c) for i, c in enumerate(self.key_list)]
        self["keylabel"].setText(res); self["progress"].setValue(int(((self.index+1)/16.0)*100))
        c_disp = ""; [c_disp := c_disp + (f"[{v}]\n" if i == self.char_index else f" {v} \n") for i, v in enumerate(self.chars)]
        self["char_list"].setText(c_disp)
    ...
    def move_char_up(self): self.char_index = (self.char_index - 1) % 6; self.update_display()
    def move_char_down(self): self.char_index = (self.char_index + 1) % 6; self.update_display()
