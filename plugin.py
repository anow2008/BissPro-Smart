...
# تحديد المسار بشكل ديناميكي ليتوافق مع بايثون 3
...
class BISSPro(Screen):
    ...
    def build_menu(self):
        ...
        for name, desc, act, icon_file in menu_items:
            full_icon_path = icon_dir + icon_file
            pixmap = None
            if os.path.exists(full_icon_path):
                pixmap = LoadPixmap(path=full_icon_path)
            
            # بناء محتوى القائمة مع الأيقونة
            res = (name, [
...
class BissManagerList(Screen):
    def __init__(self, session):
        ...
        Screen.__init__(self, session)
        self.skin = f"""
        <screen position="center,center" size="{self.ui.px(1000)},{self.ui.px(700)}" title="Key Editor">
            ...
            <eLabel text="GREEN: Edit | RED: Delete" position="{self.ui.px(75)},{self.ui.px(585)}" size="{self.ui.px(800)},{self.ui.px(40)}" font="Regular;26" transparent="1" />
        </screen>"""
