...
class BISSPro(Screen):
    def __init__(self, session):
        ...
        # Skin مع تصحيح زر Autoroll (Blue) وتنسيق الأزرار
        self.skin = f"""
    ...
    def build_menu(self):
        ...
        menu_items = [
            ("Add Key", "Manual BISS Entry", "add", "add.png"), 
            ("Key Editor", "Manage stored keys", "editor", "editor.png"), 
            ("Download Softcam", "Full update from server", "upd", "update.png"), 
            ("Autoroll", "Smart search for current channel", "auto", "auto.png")
        ]
        lst = []
    ...
    def manual_done(self, key=None):
        ...
        if self.save_biss_key(combined_id, key, info.getName()): 
            self.res = (True, f"Saved with Hash: {combined_id}")
        else: 
        ...
        try:
            ...
            if m:
                ...
                if self.save_biss_key(raw_id, ckey, info.getName()): 
                    self.res = (True, f"Key Found! Hash: {raw_id}")
                else: 
...
class BissManagerList(Screen):
    ...
    def edit_key(self):
        ...
        if curr: 
            self.old_line = curr
            parts = curr.split()
            ch_name = curr.split(";")[-1] if ";" in curr else "Unknown"
            exist_key = parts[3] if len(parts)>3 else ""
            self.session.openWithCallback(self.finish_edit, HexInputScreen, ch_name, exist_key)
...
class HexInputScreen(Screen):
    def __init__(self, session, ch_name="", exist_key=""):
        ...
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "NumberActions", "DirectionActions"], {
            ...
            "0": lambda: self.keyNum("0"), 
            "1": lambda: self.keyNum("1"), 
            "2": lambda: self.keyNum("2"), 
            "3": lambda: self.keyNum("3"), 
            "4": lambda: self.keyNum("4"), 
            "5": lambda: self.keyNum("5"), 
            "6": lambda: self.keyNum("6"), 
            "7": lambda: self.keyNum("7"), 
            "8": lambda: self.keyNum("8"), 
    ...
    def get_active_channel_data(self):
        ...
        if service:
            i = service.info()
            t = i.getInfoObject(iServiceInformation.sTransponderData)
            f = t.get("frequency", 0)
            f = f/1000 if f>50000 else f
    ...
    def update_display(self):
        ...
        self["keylabel"].setText("".join(res))
