...
import os, re, shutil, time, random, csv, ssl
from urllib.request import urlopen
from threading import Thread

# ==========================================================
# الإعدادات والمسارات - روابطك الأصلية كاملة
# ==========================================================
...
# ----------------------------------------------------------
# دالة الرسترت الأصلية (رقم 4) كاملة وبدون تغيير
# ----------------------------------------------------------
...
class BISSPro(Screen):
    def __init__(self, session):
        ...
        self.skin = f"""
        <screen position="center,center" size="{self.ui.px(1100)},{self.ui.px(780)}" title="BissPro Smart {VERSION_NUM}" backgroundColor="#101010">
    ...
    def manual_done(self, key=None):
        ...
        vpid = "%04X" % (info.getInfo(iServiceInformation.sVideoPID) & 0xFFFF) if info.getInfo(iServiceInformation.sVideoPID) != -1 else "0000"
                ...
                for row in reader:
                    if len(row) >= 2 and curr_freq in row[0]:
                        ...
                        if len(clean_key) == 16:
                            sid = "%04X" % (info.getInfo(iServiceInformation.sSID) & 0xFFFF)
                            vpid = "%04X" % (info.getInfo(iServiceInformation.sVideoPID) & 0xFFFF) if info.getInfo(iServiceInformation.sVideoPID) != -1 else "0000"
                            if self.save_biss_key(sid + vpid, clean_key, row[2] if len(row) > 2 else ch_name):
                                self.res = (True, f"Found in Sheets: {clean_key}")
                                found = True; break
            except: pass

            # --- 2. البحث في biss.txt (كودك الأصلي) ---
            if not found:
                ...
                if m:
                    ...
                    if len(clean_key) == 16:
                        sid = "%04X" % (info.getInfo(iServiceInformation.sSID) & 0xFFFF)
                        vpid = "%04X" % (info.getInfo(iServiceInformation.sVideoPID) & 0xFFFF) if info.getInfo(iServiceInformation.sVideoPID) != -1 else "0000"
                        if self.save_biss_key(sid + vpid, clean_key, ch_name):
                            self.res = (True, f"Found in GitHub: {clean_key}")
                            found = True
                
            if not found: 
                self.res = (False, "Not found for %s" % curr_freq)
        except: 
    ...
    def check_service(self):
        ...
        if service and service.info().getInfo(iServiceInformation.sIsCrypted):
            ...
            if caids and 0x2600 in caids:
                ...
                Thread(target=self.bg_auto_scan, args=(service,)).start()

    def bg_auto_scan(self, service):
        try:
            ctx = ssl._create_unverified_context()
                ...
                for row in csv.reader(resp):
                    if len(row) >= 2 and curr_freq in row[0]:
                        ...
                        if len(clean) == 16:
                            sid = "%04X" % (info.getInfo(iServiceInformation.sSID) & 0xFFFF)
                            vpid = "%04X" % (info.getInfo(iServiceInformation.sVideoPID) & 0xFFFF) if info.getInfo(iServiceInformation.sVideoPID) != -1 else "0000"
                            self.save_bg(sid+vpid, clean, row[2] if len(row)>2 else info.getName())
                            found = True; break
            except: pass
            ...
            if not found:
                ...
                m = re.search(re.escape(curr_freq) + r'[\s\S]{0,500}?(([0-9A-Fa-f]{2}[\s\t:=-]*){8})', raw_data, re.I)
                if m:
                    clean = re.sub(r'[^0-9A-Fa-f]', '', m.group(1)).upper()
                    if len(clean) == 16:
                        sid = "%04X" % (info.getInfo(iServiceInformation.sSID) & 0xFFFF)
                        vpid = "%04X" % (info.getInfo(iServiceInformation.sVideoPID) & 0xFFFF) if info.getInfo(iServiceInformation.sVideoPID) != -1 else "0000"
                        self.save_bg(sid+vpid, clean, info.getName())
        except: pass
        self.is_scanning = False

    def save_bg(self, full_id, key, name):
        target = get_softcam_path()
        try:
            lines = []
            if os.path.exists(target):
            ...
            restart_softcam_global()
            addNotification(MessageBox, f"BissPro: [ {name} ] Opened ✅", type=MessageBox.TYPE_INFO, timeout=3)
        except: pass
...
class BissManagerList(Screen):
    def __init__(self, session):
        ...
        Screen.__init__(self, session)
        self.skin = f"""
        <screen position="center,center" size="{self.ui.px(1000)},{self.ui.px(700)}" title="BissPro - Key Editor" backgroundColor="#101010">
...
class HexInputScreen(Screen):
    def __init__(self, session, channel_name="", existing_key=""):
        ...
        <screen position="center,center" size="{self.ui.px(1150)},{self.ui.px(650)}" title="BissPro - Key Input" backgroundColor="#1a1a1a">
            ...
            <widget name="l_red" position="{self.ui.px(115)},{self.ui.px(495)}" size="{self.ui.px(150)},{self.ui.px(40)}" font="Regular;{self.ui.font(26)}" transparent="1" foregroundColor="#ffffff" />
            <eLabel position="{self.ui.px(330)},{self.ui.px(500)}" size="{self.ui.px(25)},{self.ui.px(25)}" backgroundColor="#00ff00" />
            <widget name="l_green" position="{self.ui.px(365)},{self.ui.px(495)}" size="{self.ui.px(150)},{self.ui.px(40)}" font="Regular;{self.ui.font(26)}" transparent="1" foregroundColor="#ffffff" />
            <eLabel position="{self.ui.px(580)},{self.ui.px(500)}" size="{self.ui.px(25)},{self.ui.px(25)}" backgroundColor="#ffff00" />
            <widget name="l_yellow" position="{self.ui.px(615)},{self.ui.px(495)}" size="{self.ui.px(150)},{self.ui.px(40)}" font="Regular;{self.ui.font(26)}" transparent="1" foregroundColor="#ffffff" />
            <eLabel position="{self.ui.px(830)},{self.ui.px(500)}" size="{self.ui.px(25)},{self.ui.px(25)}" backgroundColor="#0000ff" />
            <widget name="l_blue" position="{self.ui.px(865)},{self.ui.px(495)}" size="{self.ui.px(230)},{self.ui.px(40)}" font="Regular;{self.ui.font(26)}" transparent="1" foregroundColor="#ffffff" />
        </screen>"""
        ...
        self["actions"] = ActionMap(["OkCancelActions", "ColorActions", "NumberActions", "DirectionActions"], {
            ...
            "yellow": self.clear_current, "blue": self.reset_all, "ok": self.confirm_char, 
    ...
    def get_active_channel_data(self):
        ...
        if service:
            ...
            self["channel_data"].setText(f"FREQ: {int(freq/1000 if freq>50000 else freq)} {pol} | SID: %04X" % (sid&0xFFFF))
...
def Plugins(**kwargs):
    return [
        PluginDescriptor(name="BissPro Smart", description="Cloud BISS Manager v1.0", icon="plugin.png", where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main),
        PluginDescriptor(where=PluginDescriptor.WHERE_SESSIONSTART, fnc=sessionstart)
    ]

def sessionstart(reason, session=None, **kwargs):
    global watcher_instance
    if reason == 0 and session is not None and watcher_instance is None:
